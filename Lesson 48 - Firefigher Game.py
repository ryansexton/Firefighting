...
#My plane already faces left so my helper function will be the opposite of the ones in the lesson
...
def bounce_plane(world: World):
    if world['plane']['x'] > 750:
        head_left(world)
    elif world['plane']['x'] < 50:
        head_right(world)
#Water Drops
WATER_DROP_SPEED = 5
...
def destroy_water_on_landing(world):
    kept = []
    for drop in world['drops']:
...
def there_are_big_fires(world:World)->bool:
    ...
    for fire in world['fires']:
        any_big_fires_so_far = fire['scale_x'] >= 0.1 or False
...
when('updating', destroy_water_on_landing)
